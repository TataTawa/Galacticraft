...
public class SpinManager {
    ...
    /**
     * Sets the spin rate for the dimension in radians per tick For example, 0.031415 would be 1/200 revolution per tick So that would be 1 revolution every 10 seconds
     */
    ...
    /**
     * This will check all blocks which are in contact with each other to find the shape of the spacestation. It also finds the centre of mass (to rotate around) and the moment of inertia (how easy/hard this is to rotate).
     * <p/>
     * If placingThruster is true, it will return false if the thruster (at baseBlock) is not in contact with the "existing" spacestation - so the player cannot place thrusters on outlying disconnected blocks and expect them to have an effect.
     * <p/>
        ...
        if (!this.clientSide) {
            ...
            for (Entity e : this.loadedEntities) {
                ...
                if ((e instanceof EntityItem || e instanceof EntityLivingBase && !(e instanceof EntityPlayer) || e instanceof EntityTNTPrimed || e instanceof EntityFallingBlock) && !e.onGround) {
                    ...
                    boolean outsideStation = entityBoundingBox.maxX < this.ssBoundsMinX || entityBoundingBox.minX > this.ssBoundsMaxX || entityBoundingBox.maxY < this.ssBoundsMinY || entityBoundingBox.minY > this.ssBoundsMaxY
                            || entityBoundingBox.maxZ < this.ssBoundsMinZ || entityBoundingBox.minZ > this.ssBoundsMaxZ;
