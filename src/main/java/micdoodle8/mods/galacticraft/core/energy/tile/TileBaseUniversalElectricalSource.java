...
public class TileBaseUniversalElectricalSource extends TileBaseUniversalElectrical {
     ...
     * 
     * The source will attempt to produce into its outputDirections whatever energy it has available, and will reduce its stored energy by the amount which is in fact used.
     * 
    ...
    /*
     * Function to produce energy each tick into the outputs of a source. If simulate is true, no energy is in fact transferred.
     * 
     * Note: even if simulate is false this does NOT reduce the source's own energy storage by the amount produced, that needs to be done elsewhere See this.produce() for an example.
     */
